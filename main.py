    ...
    def save_image(self, rsp_data, word):
        ...
        for image_info in rsp_data['data']:
            try:
                ...
                print("img+1,已有" + str(self.__counter) + "张img")
                self.__counter += 1
        return
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        # list=["1号电池","2号电池","5号电池","过期药品","易拉罐","矿泉水瓶","土豆","切过的白萝卜","切过的胡萝卜","陶瓷片","鹅卵石","砖块"]
        list = ["可回收垃圾","厨余垃圾","有害垃圾","其他垃圾"]
        for tag in list:
            crawler.start(tag, 5, 1, 50)  # 抓取关键词为 “美女”，总数为 1 页，开始页码为 2，每页30张（即总共 2*30=60 张）
